<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-slate-950">Add Train</h2>
        </div>
    </x-slot>

    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('trains.store') }}" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf

            <div class="grid gap-5">
                <div>
                    <x-input-label for="train_name" value="Train Name" />
                    <x-text-input id="train_name" name="train_name" type="text" class="mt-2 block w-full" value="{{ old('train_name') }}" required autofocus />
            ...
            <div class="mt-6 flex items-center justify-end gap-3">
                ...
                <x-primary-button>Save Train</x-primary-button>
            </div>
        </form>
    </div>
</x-app-layout>
