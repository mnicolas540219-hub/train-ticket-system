<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-slate-950">Edit Route</h2>
        </div>
    </x-slot>

    @php
        $stops = old('stops', $route->stops->pluck('station_name')->values()->all());
        $stops = count($stops) > 0 ? $stops : [''];
    @endphp

    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('routes.update', $route) }}" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm" x-data="{ stops: @js($stops) }">
            @csrf
            @method('PUT')
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <x-text-input id="origin" name="origin" type="text" class="mt-2 block w-full" value="{{ old('origin', $route->origin) }}" required autofocus />
                    <x-input-error :messages="$errors->get('origin')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="destination" value="Destination" />
                    <x-text-input id="destination" name="destination" type="text" class="mt-2 block w-full" value="{{ old('destination', $route->destination) }}" required />
            ...
            <div class="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="text-sm text-slate-500">Keep the stops in travel order from origin to destination.</p>
                    </div>
