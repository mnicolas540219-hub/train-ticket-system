<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-slate-950">Add Schedule</h2>
        </div>
    </x-slot>

    <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <form method="POST" action="{{ route('schedules.store') }}" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <select id="train_id" name="train_id" class="mt-2 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-700 focus:ring-slate-700" required>
                        ...
                        @foreach ($trains as $train)
                            <option value="{{ $train->id }}" @selected(old('train_id') == $train->id)>{{ $train->train_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="route_id" name="route_id" class="mt-2 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-700 focus:ring-slate-700" required>
                        ...
                        @foreach ($routes as $route)
                            <option value="{{ $route->id }}" @selected(old('route_id') == $route->id)>{{ $route->origin }} to {{ $route->destination }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="departure_time" name="departure_time" type="time" step="60" class="mt-2 block w-full" value="{{ old('departure_time') }}" required />
                    <x-input-error :messages="$errors->get('departure_time')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="arrival_time" value="Daily Arrival Time" />
                    <x-text-input id="arrival_time" name="arrival_time" type="time" step="60" class="mt-2 block w-full" value="{{ old('arrival_time') }}" required />
                    <x-input-error :messages="$errors->get('arrival_time')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="fare" value="Fare" />
                    <x-text-input id="fare" name="fare" type="number" step="0.01" min="0" class="mt-2 block w-full" value="{{ old('fare') }}" required />
                    <x-input-error :messages="$errors->get('fare')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="available_seats" value="Available Seats" />
                    <x-text-input id="available_seats" name="available_seats" type="number" min="0" class="mt-2 block w-full" value="{{ old('available_seats') }}" required />
            ...
            <div class="mt-6 flex items-center justify-end gap-3">
                ...
                <x-primary-button style="position:relative;z-index:10">Save Schedule</x-primary-button>
            </div>
        </form>
    </div>
</x-app-layout>
